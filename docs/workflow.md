...
> **What this page covers, and what this collection ships.** The lifecycle below is the whole trajectory, including stages driven by skills that are not in this repository yet. Twelve are named here and not included: the two stage drivers `/first-mile` and `/last-mile`, the whole-run driver `/gauntlet`, `/implement-frame`, the guards `/measure`, `/rethink` and `/poc`, and `/whatnext`, `/recap-wherearewe`, `/test-mutation`, `/obsidian-update`, `/obsidian-init`. Until they land, read those stages as a description of the shape rather than as commands you can run. Everything else named below ships here — as a skill in `dotclaude/skills/` or a command in `dotclaude/commands/`.

```mermaid
flowchart LR
    subgraph FM ["/first-mile drives these three"]
        O["Orient<br/>/wherearewe · /familiarize<br/>/analysis"] --> D["Design<br/>/dev-workflow-process · /rethink<br/>collab family"] --> T["Track<br/>/github-issue · /addendum<br/>/obsidian · /merge-3-way-split"]
    end
    T --> I["Implement<br/>/implement-frame<br/>one unit at a time, tests as it goes"]
    I --> V["Verify<br/>/test-checklist · /test-mutation<br/>tester agents"]
    subgraph LM ["/last-mile drives these three"]
        G["Gate<br/>/github-acceptance-check<br/>/prepcommit · /version-bump"] --> C["Commit<br/>/commit"] --> S["Ship<br/>git tag · /github-release"]
    end
    V --> G
    ...
    ST["Steer (re-entrant)<br/>/whatnext · /recap-wherearewe"] -.-> O & D & I & V & G
    M["/measure · /poc<br/>ground truth BEFORE a decision"] -.- D
    ...
    GN["/gauntlet drives the whole run<br/>first-mile → implement-frame → verify → last-mile → reflect<br/>on-disk ledger · stops at every human gate"] -.- FM & I & LM
```

Two drivers bracket the middle of the diagram, and they are mirror images. **`/first-mile`** carries a question from "is this worth designing?" to an approved design with a numbered unit list, and stops there. **`/last-mile`** carries finished work from "it works" to "it shipped", and stops at every gate a person must clear. Between them sits the one thing neither does: the build, which **`/implement-frame`** does one unit at a time. **`/gauntlet`** is the driver of drivers — it runs a whole slice through all of it in order, and it is the only skill on this page that a person must start by hand.
...
## 2. Design and 3. Track — driven by `/first-mile`

**`/first-mile`** is the counterpart of `/last-mile`: one command that walks the design stage for you and ends at the approved design — never starting the build, never committing. Its first written line is a contract question: *does anything durable build against this decision?* A "no" must name what was checked, and then a five-minute question exits in one turn with one quick question instead of an hour of analysis. A "yes" runs the walk: orient (below), an entry proposal that ends the turn so you can say "go", the free checks on what the request itself asserted — `/rethink` on every "there is no X", `/measure` on every number nobody ran and on the population the decision will rest on — then the `/dev-workflow-process` analysis with two columns added to its candidate table: for every candidate, *the cheapest observation that could kill it*, written before any evidence is spent, and *the level of evidence that settled it* (argued — counted, never merely labelled; read — a line citation plus what would have moved it further; ran it). At most one `/poc`. A consultation when the analysis does not converge. One gate. Then tracking. It ends by naming what the design became: a unit list for `/implement-frame`, spawned designs, an issue, or a deferral with its reopen condition in the person's own words. Every design leaves a provenance section — each step as run or skipped, with the output of a small transcript lint that lists the session's real skill dispatches, so the record cannot claim a step that did not happen. Typed by a person, the person chooses the depth (a short form exists for narrow contracts); reached for by the model on a mechanism choice, it asks one question and stops.

The stages it drives, in order:
...
Three active tools guard the design against its own assumptions — the first two for things that exist, the third for things that do not yet:
...
- **`/poc`** — settle whether a *proposal* would work by building the smallest thing that could show it **fails**: a prediction written first, a control arm designed to fail (a passing control invalidates every result, including the flattering ones), and a pass criterion decided before anything runs. This is for the thing that does not exist yet, where `/measure` has no population to survey and arguing further will not settle it. It writes only under `tests/one-offs/`, keeps the artifact, and records a refuted prediction as a success — it cost an hour and saved building the wrong thing.

Branches out of design (the **Track** stage):
...
- **Continuously**: capture progress with **`/addendum`** (append to the active design doc), **`/obsidian`** (vault notes), or **`/docidea`** (lightweight idea capture; in newer builds of the toolkit it has joined the obsidian family as `/obsidian-idea`)

## 4. Implement — `/implement-frame`

The one stage neither driver performs. **`/implement-frame`** does one unit of work at a time under anchors: a baseline suite count taken before the first change, per-iteration testing, and a triage table that is allowed to name the *test* as the thing that is wrong. It can suspend for design, measurement, or consultation and resume without losing its position, continues to the next unit when one is finished, and raises the testing-depth question as the gate it is. It never commits, tags, or pushes. Enter it alone when the next step is clear and the work is code, when `/whatnext` lands on Implement, or as `/gauntlet`'s implement station.
...
## 6. Gate, 7. Commit, 8. Ship — driven by `/last-mile`
...
## The whole run: `/gauntlet`

**`/gauntlet`** drives one slice of work through everything above, in order, by dispatching the skills rather than paraphrasing them: `/first-mile` for the design, `/implement-frame` for the build, the verify skills, `/last-mile` for the ship, the postmortem family and `/whereweare` to reflect. It keeps an on-disk ledger of where the run is, so a run can be resumed by another session, and it stops at every gate that needs a person — design approval, commit, push, release. It has two modes: attended, where each gate is presented once and the turn ends; and unattended, where the soft gates (design, scope, testing depth, version bump) take their recommendation and are logged for review, while the hard ones (commit, push, release, closing an issue) still wait. It is human-started only — the model never invokes it — and it is the right tool when a slice should leave a complete artifact trail without the sequence being renegotiated in prose each time. Its own rule for the skills it dispatches: it may only log a dispatch in the turn it actually made one, because everything the driver offers rests on its ledger being true.
